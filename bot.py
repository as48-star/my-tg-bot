...
API_ID = 21727773  # <--- Agar ye galat hai toh apna asli API_ID likhein
...
async def start_handler(event):
    ...
    if not await check_user_joined(event.sender_id):
        ...
        return await event.reply("❌ **Access Denied!**\n\nBot use karne ke liye pehle hamare backup channel ko join karein.", buttons=buttons)
    await event.reply("👋 **Bot Online!**\n\nAb aap link bhej sakte hain, main sequence fetch kar lunga.")
...
async def handler(event):
    ...
    if not m:
        return await event.reply("❌ Link sahi nahi hai!")
    ...
    status = await event.reply("⏳ **Sequence Processing...** (Fetching next 10 posts)")
    ...
    for i in range(10):
        ...
        try:
            ...
            await asyncio.sleep(1.5) # Safe gap
